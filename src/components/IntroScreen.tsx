...
export default function IntroScreen({ onStart, onUpload, isLoading }: Props) {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-12">
      ...
      <a
        href="/palm"
        className="mt-6 block rounded-xl border border-dashed border-[var(--color-accent)] bg-[color-mix(in_srgb,var(--color-accent)_4%,transparent)] p-4 hover:bg-[color-mix(in_srgb,var(--color-accent)_8%,transparent)] transition-colors"
      >
        <p className="text-xs font-semibold uppercase tracking-wider text-[var(--color-accent)] mb-1">
          手相鏡 · 손금 도구 (β)
        </p>
        <p className="text-sm text-[var(--color-primary)]">
          손바닥을 카메라에 보여주면 손 모양 분류(불·땅·공기·물)와 8 손금선의 위치
          안내 →
        </p>
      </a>
    </div>
